        ...
        UserSelectionPage.UserSelectionListener userSelectionListener = new UserSelectionPage.UserSelectionListener() {
          ...
          public void onAutomaticChatRequested(String targetUser, UserSelectionPage userSelectionPage) {
            ...
            try {
              ...
              JOptionPane.showMessageDialog(userSelectionPage, "Connection initialization failed", "Error",
                  JOptionPane.ERROR_MESSAGE);
              if (chatClient != null)
                chatClient.dispose();
            } catch (Exception e) {
              logger.severe("Error while initializing connection: " + e.getMessage());
              JOptionPane.showMessageDialog(userSelectionPage, "Error while initializing connection: " + e.getMessage(),
                  "Connection error", JOptionPane.ERROR_MESSAGE);
              if (chatClient != null)
                chatClient.dispose();
            }
            ...
            MenuPage.MenuListener menuListener = new MenuPage.MenuListener() {
              ...
              public void onKeysExchangedAndProceed(CryptoManager readyCryptoManager, MenuPage menuPageInstance) {
                ...
                try {
                  ...
                  if (!connected) {
                    logger.severe("Impossible connecting to server.");
                    JOptionPane.showMessageDialog(null, "Impossible connecting to server.", "Connection error",
                        JOptionPane.ERROR_MESSAGE);
                    chatClient.dispose();
                    System.exit(1);
                    return;
                ...
                try {
                  ...
                  logger.severe("Error while initializing connection: " + e.getMessage());
                  JOptionPane.showMessageDialog(null, "Error while initializing connection: " + e.getMessage(),
                      "Connection error", JOptionPane.ERROR_MESSAGE);
                  System.exit(1);
                }
              }

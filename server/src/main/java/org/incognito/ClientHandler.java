...
public class ClientHandler implements Runnable {
    ...
    public void run() {
        try {
            ...
            if (this.username == null) { // If it was not possible to register the username
                logger.warning("Failed to register username for client " + socket.getRemoteSocketAddress() + ". Closing connection.");
                closeConnection();
                return;
            }
            ...
            if ("Connection reset".equals(e.getMessage()) || "Socket closed".equals(e.getMessage()) || e.getMessage().contains("Socket write error")) {
                logger.info("Client " + (username != null ? username : socket.getRemoteSocketAddress()) + " connection issue: " + e.getMessage());
            } else {
                logger.severe("IOException for client " + (username != null ? username : socket.getRemoteSocketAddress()) + ": " + e.getMessage());
                e.printStackTrace();
            }
        } catch (ClassNotFoundException e) {
            logger.severe("ClassNotFoundException from client " + (username != null ? username : socket.getRemoteSocketAddress()) + ": " + e.getMessage());
        } catch (Exception e) { // Catch-all for unexpected exceptions
            logger.severe("Unexpected error in ClientHandler for " + (username != null ? username : socket.getRemoteSocketAddress()) + ": " + e.getMessage());
            e.printStackTrace();
        }
        finally {
    ...
    public void send(Object message) {
        if (socket.isClosed() || outputStream == null) {
            logger.warning("Cannot send message to " + (username != null ? username : "disconnected client") + ", socket/stream closed.");
            return;
        }
        try {
